...
if (isLinux()) {
  ...
  ipcMain.on('seeked', (_event, arg) => {
    // Send the position from SysMusic to MPRIS on manual seek
    setTimeout(() => {
...
if (isWindows() && isWindows10()) {
  ...
  Controls.displayUpdater.musicProperties.title = 'SysMusic';
      ...
      windowsStorageStreams.RandomAccessStreamReference.createFromUri(
        new windowsFoundation.Uri(
          arg.image.includes('placeholder')
            ? 'https://raw.githubusercontent.com/jeffvli/SysMusic/main/src/img/placeholder.png'
            : arg.image
        )
      );
